...
public:
  ...
  virtual void close();
  virtual void inputKey(const KeyState &ks) {
    controller_->quit(0);
  }
private:
...
Visuport::Visuport() : is_playing_(false),
  camera_(vec3f(0.f,-10.f,10.f),
          vec3f(0),
          vec3f(0,0,1)), sampleAngle_(0) {}
void Visuport::init(IViewportController *system) {
  controller_ = system;
  render_ = new Render;
  ground_ = new Ground(8, 8.f);
